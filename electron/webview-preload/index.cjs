...
/* ── 1. 基础指纹 ─────────────────────────────────── */
function injectNavigator(config) {
    const nav = window.navigator;
    const proto = Navigator.prototype;
    const rewrite = (key, getter) => {
        try {
    ...
    if (config.userAgent)
        rewrite('userAgent', () => config.userAgent);
...
/* ── 2. Screen / Window ──────────────────────────── */
function injectScreen(config) {
    if (!config.resolution || config.resolution === '跟随系统')
        return;
    let width = 1920;
    let height = 1080;
...
/* ── 3. Canvas 噪音 ───────────────────────────────── */
function injectCanvasNoise(enabled) {
    if (!enabled)
        return;
    ...
    HTMLCanvasElement.prototype.toDataURL = function (...args) {
        ...
        if (ctx)
            ctx.getImageData(0, 0, 1, 1); // trigger noise injection once
...
function injectWebRTC(config) {
    ...
    window.RTCPeerConnection = function (...args) {
        ...
        const patchSdp = (sdp) => sdp.replace(ipRegex, fakeIP);
...
/* ── 5. AudioContext 噪音 ─────────────────────────── */
function injectAudioNoise(enabled) {
    if (!enabled)
        return;
...
/* ── 6. Geolocation ──────────────────────────────── */
function injectGeolocation(mode) {
    ...
    geo.getCurrentPosition = function (_success, error, _options) {
        denied(error);
    };
    if (mode === 'disable') {
        geo.watchPosition = function (_success, error, _options) {
            denied(error);
            return 0;
        };
        if (geo.clearWatch)
...
/* ── 9. Timezone ──────────────────────────────────── */
function injectTimezone(config) {
...
function injectFonts() {
    try {
        ...
        if (window.queryLocalFonts) {
            ;
            window.queryLocalFonts = async () => fonts.map((family) => ({ family, fullName: family, postscriptName: family, style: 'Regular' }));
        }
        if (document.fonts && document.fonts.check) {
            const origCheck = document.fonts.check.bind(document.fonts);
            document.fonts.check = (font, text) => origCheck(font, text);
        }
    }
    catch {
        // ignore
    ...
    try {
        ...
        navigator.permissions.query = async (descriptor) => {
            ...
            if (denied.includes(name)) {
                return { state: 'denied', onchange: null, addEventListener: () => { }, removeEventListener: () => { } };
            }
...
function disableWorkersAndChannels() {
    try {
        if (window.ServiceWorkerContainer) {
            ;
            window.ServiceWorkerContainer.prototype.register = () => Promise.reject(new Error('ServiceWorker disabled'));
            window.ServiceWorkerContainer.prototype.ready = new Promise(() => { });
        }
    }
    catch {
        // ignore
    ...
    try {
        if (window.BroadcastChannel) {
            ;
            window.BroadcastChannel.prototype.postMessage = () => { };
            window.BroadcastChannel.prototype.addEventListener = () => { };
        }
    }
    catch {
        // ignore
...
function disableSensitiveApis() {
    ...
    try {
        if (navigator.clipboard && navigator.clipboard.readText) {
            ;
            navigator.clipboard.readText = () => Promise.reject(new DOMException('No user activation', 'NotAllowedError'));
        }
    }
    catch {
        // ignore
...
function isolateStorage(partition) {
    try {
        ...
        localStorage.setItem = (key, value) => origSetItem(prefix + key, value);
        localStorage.getItem = (key) => origGetItem(prefix + key);
        localStorage.removeItem = (key) => origRemoveItem(prefix + key);
...
function injectWebdriver(enabled) {
    if (enabled === false)
        return;
    try {
...
function injectChrome(enabled, os) {
    if (enabled === false)
        return;
    try {
        const chromeObj = {
...
function injectOuterDimensions(config) {
    if (config.fakeOuterSize === false)
        return;
    if (!config.resolution || config.resolution === '跟随系统')
        return;
    const m = config.resolution.match(/(\d+)\s*x\s*(\d+)/i);
    ...
    try {
        if (window.speechSynthesis) {
            ...
            window.speechSynthesis.getVoices = () => [
                ...
                { voiceURI: 'Microsoft Yaoyao', name: 'Microsoft Yaoyao', lang: 'zh-CN', localService: false, default: false },
            ];
        }
    }
    catch {
...
async function init() {
    try {
        ...
        const partition = document.querySelector('webview')?.partition || '';
        // 启动聊天抓取（不依赖指纹配置；partition 在 guest page 中可能为空字符串）
        startAutoReplyScraper(partition);
        const config = await electron_1.ipcRenderer.invoke('fingerprint:get', partition);
        if (!config)
            return;
